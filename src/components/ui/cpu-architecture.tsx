...
export const CpuArchitecture = ({
  ...
  <svg className={cn("text-muted", className)} width={width} height={height} viewBox="0 0 200 100">
    ...
    {[
      ["cpu-mask-1", "cpu-line-1", "cpu-blue-grad"],
      ["cpu-mask-2", "cpu-line-2", "cpu-yellow-grad"],
      ["cpu-mask-3", "cpu-line-3", "cpu-pinkish-grad"],
      ["cpu-mask-4", "cpu-line-4", "cpu-white-grad"],
      ["cpu-mask-5", "cpu-line-5", "cpu-green-grad"],
      ["cpu-mask-6", "cpu-line-6", "cpu-orange-grad"],
      ["cpu-mask-7", "cpu-line-7", "cpu-cyan-grad"],
      ["cpu-mask-8", "cpu-line-8", "cpu-rose-grad"],
    ].map(([mask, line, grad]) => (
      <g key={mask} mask={`url(#${mask})`}>
        <circle className={`cpu-architecture ${line}`} cx="0" cy="0" r="8" fill={`url(#${grad})`} />
      </g>
    ))}
    ...
    <g>
      ...
      <text x="89" y="52.5" fontSize="6" fill={animateText ? "url(#cpu-text-gradient)" : "white"} fontWeight="600" letterSpacing="0.05em">
        {text}
      </text>
    </g>
